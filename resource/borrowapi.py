...
def return_book(data, borrowing_id):
    borrowing_record = BorrowingModel.query.filter(borrowing_id==borrowing_id).first()

    if not borrowing_record:
        abort(404, message="not found")
    borrowing_record.if_return = True

    borrowing_record.returning_time = datetime.now()

    db.session.commit()

    shelf_record = ShelfModel.query.filter_by(borrowing_id=borrowing_record.borrowing_id).first()
    if shelf_record:
